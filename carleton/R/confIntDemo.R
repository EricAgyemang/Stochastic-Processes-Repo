confIntDemo <-
function(distr = "normal", n = 20, conf.level = .95)
{  #14 October 2013, LMC
    
   if (n > 100000) stop("Choose smaller sample n")
   if (n < 2) stop("Choose larger sample size")  
   ...
   for (i in 1:100)
   {
      ...
      {
      se <- sd(x)/sqrt(n)
      lower <- xbar-abs(qt(alpha/2, n - 1))*se
      upper <- xbar+abs(qt(alpha/2, n - 1))*se
     }
      temp[i,1] <- lower
      temp[i,2] <- upper
